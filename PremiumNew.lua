...
local timeAPI = "https://timeapi.io/api/timezone/zone?timeZone=Asia%2FJakarta"
local interval = 1

local notified = false
local kicked = false

local expiryUnix = nil

-- Ambil waktu saat ini dari API
local function getCurrentDateTime()
	local success, response = pcall(function()
		return game:HttpGet(timeAPI)
	end)
	...
	if not ok or not data.currentLocalTime then return nil end

	local year, month, day, hour, minute, second = data.currentLocalTime:match("^(%d+)%-(%d+)%-(%d+)T(%d+):(%d+):(%d+)")
...
-- Konversi string expired ke unix
local function convertExpiryToUnix(expiryString)
	local d, m, y, h, min, s = expiryString:match("(%d%d)%-(%d%d)%-(%d%d%d%d)%s+(%d%d):(%d%d):(%d%d)")
...
local function validateInitialLicense()
	local success, data = pcall(function()
		return game:HttpGet(pasteURL)
	end)

	if not success or not data then
		warn("Failed to fetch license data.")
		return false
	end

	for line in data:gmatch("[^\r\n]+") do
		local username, exp = line:match("^(.-)|(%d%d%-%d%d%-%d%d%d%d %d%d:%d%d:%d%d)$")
		if username and exp and username == LocalPlayer.Name then
			expiryUnix = convertExpiryToUnix(exp)
			if not expiryUnix then
				warn("Failed to parse expiry date.")
				return false
			end

			if NotifySuccess then
				NotifySuccess("Lisensi Valid", "User: " .. LocalPlayer.Name)
				NotifySuccess("Expired On", exp)
			end
	...
	LocalPlayer:Kick("License not found!")
	kicked = true
	return false
end

local initialValid = validateInitialLicense()
if not initialValid then return end

task.spawn(function()
	while not kicked do
		task.wait(interval)

		local currentTime = getCurrentDateTime()
		if not currentTime then
			warn("Gagal mengambil waktu dari internet. Mencoba lagi...")
		elseif expiryUnix and currentTime >= expiryUnix then
			LocalPlayer:Kick("License has expired!")
			kicked = true
			break
		end
	end
...
local function checkBlacklist()
	...
	local blacklist = string.upper(result)
	local username = string.upper(LocalPlayer.DisplayName)

	if string.find(blacklist, username) then
		if not kicked then
			LocalPlayer:Kick("HAHAHAHA THIS IS TALON, STOP CHEAT!!!")
			kicked = true
		end
	else
		if not notified then
...
local function saveOriginalPosition()
	repeat task.wait() until workspace:FindFirstChild("Characters") and workspace.Characters:FindFirstChild(LocalPlayer.Name)
	local char = workspace.Characters[LocalPlayer.Name]
	local hrp = char:FindFirstChild("HumanoidRootPart")
	if hrp then
...
local function teleportToEvent(model)
	...
	if char and char:FindFirstChild("HumanoidRootPart") then
		char.HumanoidRootPart.CFrame = model.PrimaryPart.CFrame + Vector3.new(0, 13, 0)
	end
end
	...
	if props then
		for _, model in ipairs(props:GetChildren()) do
			if model:IsA("Model") and model.PrimaryPart then
				knownEvents[model.Name:lower()] = model
			end
		end
	end
end
...
local function checkForEvent()
	if not autofarmEventEnabled then return end

	for name in pairs(eventNames) do
		local model = knownEvents[name]
		if model and not eventTeleported then
			currentEvent = name
			eventTeleported = true
			saveOriginalPosition()
			teleportToEvent(model)

			if floatingPlat then floatingPlat(true) end
			if NotifySuccess then NotifySuccess("Event Detected", "Teleported to event: " .. name:upper()) end
			return
		end
	end

	if eventTeleported and currentEvent and not knownEvents[currentEvent] then
		eventTeleported = false
		currentEvent = nil
		returnToOriginalPosition()

		if floatingPlat then floatingPlat(false) end
		if NotifyInfo then NotifyInfo("Event Ended", "Returned to original position.") end
	end
end
...
local WebhookURL = "https://discord.com/api/webhooks/1398155779059744931/dMJWFs3LhU9XetoQr49i4LCiFMXHG5qZsIterXxX-zCqFLy_pgiT5crDPkqZxGfJNdQu"      
